...
- **Tocar música no Spotify** (conta Premium, conectada uma vez em
  ...
  (`/api/spotify/*`).
- **Encontrar, conectar e controlar aparelhos da mesma rede** (aba Aparelhos, voz, chat ou API `/api/devices`):
  procura qualquer aparelho que se anuncia no Wi-Fi, conecta e classifica (TV, player de streaming, caixa de
  som, lâmpada, computador, roteador...). Os que têm controle — hoje TVs e players: Fire TV/Android TV (ADB,
  completo, liga/desliga a TV pelo HDMI-CEC), LG webOS, Samsung, Roku e os que só anunciam DIAL (ex.:
